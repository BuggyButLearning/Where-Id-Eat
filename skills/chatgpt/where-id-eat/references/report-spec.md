...
- Search official restaurant pages first, then reputable editorial, reservation, social, and current business-listing sources.
- Do not use an SVG fallback merely because a photo is inconvenient to retrieve.
- If no usable photo can be found after at least three documented photo-source checks, generate a simple neutral SVG fallback for that recommendation.
